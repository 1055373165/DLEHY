...
- 当前真正缺的是“把这条 repair worker contract 继续接到可插拔的 repair-agent registry”，让 runtime 能按 worker hint / contract version 选择具体 repair agent，而不是只有一个固定的 in-process worker。
...
- 把 repair worker contract 继续推进到 pluggable repair-agent registry / selection
