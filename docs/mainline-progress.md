...
- 当前真正缺的是“把这条 distinct repair worker registry 继续接到真正独立的 repair-agent adapter / executor”，让 runtime 不只是按 hint/version 选到不同本地类，而是能把 repair lane 交给不同 repair executors。
...
- 把 repair worker registry 继续推进到 genuinely independent repair-agent adapters / selection
