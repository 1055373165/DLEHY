...
- 当前真正缺的是“runtime 自己持有 repair dispatch / execution 领取链，而不是只把 repair plan 留在 JSON 里”。
...
- 把 repair dispatch 绑定到真正可领取的 runtime repair lane，而不是只停在 proposal status detail
- 把后续 patch validation、bundle publish/rollback、replay scope 继续统一绑定到 repair dispatch lineage
