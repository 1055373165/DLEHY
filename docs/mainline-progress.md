...
- 当前真正缺的是“把这条 repair executor contract 从第一条 subprocess-backed export lane 继续扩到更广的 remote / transport-backed executors”，让 runtime 不只是把单条 repair lane 交给本地子进程，而是能把更多 repair lanes 交给真正独立的 repair agents。
...
- 把 repair-executor contract 从第一条 subprocess-backed lane 继续推进到更广的 remote / transport-backed repair executors
