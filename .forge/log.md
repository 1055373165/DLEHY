...
- 2026-04-04 02:04:04 +0800 | resolved stale `.forge` and handoff merge-conflict drift in favor of the active translate-agent readiness mainline; removed obsolete batch-64/65/66 canonical-IR branch truth from the live ledger
- 2026-04-04 02:04:04 +0800 | folded already-measured current readiness, slice-first pilot evidence, and expansion-wave1 `overall go` back into a single resumable `.forge` truth
- 2026-04-04 02:04:04 +0800 | reprioritized the next execution stack so Forge v2 now takes two explicit user-provided paper probes first: `pdf-attention-single-column-002` and `pdf-raft-atc14-001`
- 2026-04-04 02:04:04 +0800 | created gold-label stubs and corpus-expansion draft entries for the two user-provided paper samples so the next resume can annotate and probe them without reopening task discovery
- 2026-04-04 02:25:24 +0800 | annotated and measured `pdf-attention-single-column-002`; the user-prioritized single-column Attention variant lands on parser/export `overall go` and widens L3 paper-variance evidence without spending translation tokens
- 2026-04-04 18:13:12 +0800 | wrote a project-memory budget rule into `.forge` truth and handoff docs: during test/readiness work the maximum translation-bearing test unit is one chapter or an equivalent chapter-sized slice, never a whole-book test
- 2026-04-04 18:25:19 +0800 | converted the first `L4` OCR-heavy scanned-book sample into a measured `go` by adding slice-scoped PDF OCR probing, bounded OCR runtime control, and scanned-page image-block materialization so full-page scans remain `image/protect` with `pdf_original_image` parity
- 2026-04-04 18:32:47 +0800 | measured the second `L4` OCR-heavy scanned-book sample `pdf-self-observation-zh-001` as `go` through the same bounded slice-scoped OCR/parser/export path, then shifted the default next expansion slice back to the EPUB-family queue because L4 now has two measured successes
- 2026-04-04 19:06:00 +0800 | hardened Forge v2 stop legality at the Codex hook layer by adding a repo-owned stop guard that reads `.forge/STATE.md`, blocks illegal `Stop` events from active continuation states such as `ready_for_dispatch`, and only delegates stop-time auto-commit after legality passes
- 2026-04-04 20:02:00 +0800 | annotated and measured `epub-agentic-theories-001`; the first post-OCR EPUB-family expansion sample lands on chapter-scale parser/export `go` and widens `L1` with deeper heading nesting, archive-backed figures, and ordered-list-heavy content
- 2026-04-04 20:02:00 +0800 | fixed EPUB chapter heading normalization for chapter-title-plus-h2/h3/h4 books and repaired benchmark list matching so section-anchored list gold labels fall back to role-compatible list items instead of mismatching headings
- 2026-04-04 21:39:00 +0800 | fixed the Forge v2 stop-hook output contract: the repo-owned guard now returns Codex-compatible top-level `continue + decision + reason` JSON for `Stop`, eliminating the invalid-stop-hook-JSON failure while preserving illegal-stop blocking
- 2026-04-04 21:39:00 +0800 | moved `.forge/STATE.md` to the explicit `review_stop_hook_protocol_fix` boundary so this protocol-only repair can hand off cleanly without pretending the translate-agent readiness queue is complete
- 2026-04-04 22:08:00 +0800 | resynced `.forge` and handoff truth after the stop-hook protocol repair: both `pdf-attention-single-column-002` and `pdf-raft-atc14-001` remain explicitly recorded as widened `L3` paper-variance success cases, and STATE.md now uses the legal `awaiting_review` boundary instead of the illegal protocol-only stop state
- 2026-04-05 01:09:00 +0800 | moved `.forge/STATE.md` to the legal `paused` boundary because the user explicitly delegated the repo-reading interview-analysis task overnight and is not expected to answer further until waking; active translate-agent next_items remain preserved for the next Forge v2 resume
- 2026-04-04 02:27:44 +0800 | annotated and measured `pdf-raft-atc14-001`; the user-prioritized double-column systems-paper sample lands on `no_go` with measured blockers in heading hierarchy and reading order
- 2026-04-04 02:34:55 +0800 | hardened academic inline heading recovery for question-style and lowercase-tail titles, preventing `Introduction Consensus` over-capture and recovering `3 What’s wrong with Paxos?`
- 2026-04-04 02:34:55 +0800 | reran `pdf-raft-atc14-001` after the heading fix; heading accuracy improved, and the remaining blocker narrowed to `reading_order_accuracy_below_threshold`
- 2026-04-04 16:36:11 +0800 | hardened PDF academic-paper profiling so proceedings-cover layouts with the real paper title on page 2 still enter the `academic_paper` recovery lane
- 2026-04-04 16:36:11 +0800 | fixed benchmark caption-linkage scoring to fall back to block-level `source_anchor` when materialized block-id linkage is absent but source-anchor linkage is present
- 2026-04-04 16:36:11 +0800 | reran `pdf-raft-atc14-001`; the user-prioritized double-column systems-paper probe now lands on measured `go`, clearing the paper-variance queue and returning the default next slice to `L4` OCR-heavy expansion
- 2026-04-04 23:03:37 +0800 | tightened stop legality from substring matching to explicit allowlisted states so `review_stop_hook_protocol_fix` remains a continuation state
- 2026-04-04 23:03:37 +0800 | patched the legacy Codex-home Stop auto-commit entry so cached sessions also block illegal Forge v2 stops before stop-time auto-commit
- 2026-04-04 23:03:37 +0800 | annotated `epub-managing-memory-001`, executed EPUB family wave 2, and recorded measured `go` as widened `L1` evidence
- 2026-04-04 23:41:58 +0800 | corrected the `epub-managing-memory-001` live pilot to the intended `OEBPS/ch05.html` chapter, executed two bounded chapter slices to `16/31` translated packets with zero review-required sentences, and fixed chapter-smoke continuation to select the next built packet window instead of replaying the first slice
- 2026-04-04 23:43:30 +0800 | completed the `epub-managing-memory-001` `OEBPS/ch05.html` live chapter pilot to `31/31` translated packets, finished review with zero issues, and emitted both review-package and bilingual exports from the bounded chapter-scale continuation chain
- 2026-04-05 00:12:36 +0800 | switched the default translation target from DeepSeek to OpenRouter free Qwen by updating `.env` to `qwen/qwen3.6-plus:free` on `https://openrouter.ai/api/v1`, preserved the `openai_compatible` adapter path, and verified the route with a minimal live structured-output probe
- 2026-04-05 00:58:41 +0800 | started the first real chapter-scale live pilot on the OpenRouter free Qwen route (`epub-agentic-theories-001` Chapter 12), completed two bounded slices (`16` packets, `0` review-required, `0` provider failures), and recorded the first measured latency profile as usable but significantly slower than prior routes
