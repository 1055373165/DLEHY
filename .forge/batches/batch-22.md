...
Add an explicit repair-executor contract so the executor-owned `REPAIR` lane can choose an executor
layer independently from the repair-agent adapter layer.

## Goals

- Introduce a deterministic `RuntimeRepairExecutorRegistry`.
- Keep current execution mode `in_process`, but make it explicit in repair dispatch input bundles.
- Make unknown executor hints or versions fail through the repair work-item lifecycle.
- Preserve adapter metadata and add executor metadata so remote executors can later slot in behind
  the same contract.
...
.venv/bin/python -m py_compile \
  ...
  tests/test_runtime_repair_executor.py \
