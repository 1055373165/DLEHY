...
- The next dependency-closed slice is `transport-backed repair executors`.
- Goal: keep the executor contract deterministic, then extend the first subprocess-backed export repair executor into broader remote or transport-backed repair executors instead of only local agent-backed execution.
