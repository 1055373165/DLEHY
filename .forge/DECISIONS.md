...
- The next dependency-closed slice is `repair agent contract hardening`.
- Goal: keep the executor-owned repair lane deterministic, then split out a dedicated repair worker contract so runtime can dispatch repair work-items without embedding the whole repair flow inside `DocumentRunExecutor`.
